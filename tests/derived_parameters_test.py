...
class TestAltitudeAGL(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        alt_rad = P(name='Altitude Radio', array=(np.minimum(6000,
                                                             (1.0-np.cos(np.array(range(100))*3.14/50))*4000 + np.random.rand(100)*300)),
                                           frequency = 2)
...
class TestControlColumnForceCapt(unittest.TestCase):
    ...
    def test_control_column_force_capt(self):
        ...
        capt = np.ma.arange(20, 28, 0.1)
        capt[6] = np.ma.masked
        capt[76] = np.ma.masked
        ccf.derive(
            ControlColumnForce('Control Column Force (Capt) Recorded', capt),
            ControlColumnForce('Control Column Force (FO) Recorded', np.ma.ones(80) * 25))
        self.assertEqual(ccf.array.round(decimals=1).tolist(), [None] * 52 + [25.2, 25.3, 25.4, 25.5, 25.6, 25.7, 25.8, 25.9, 26.0, 26.1, 26.2, 26.3, 26.4, 26.5, 26.6, 26.7, 26.8, 26.9, 27.0, 27.1, 27.2, 27.3, 27.4, 27.5, None, 27.7, 27.8, 27.9])
    ...
    def test_control_column_force_capt_neg(self):
        ...
        self.assertEqual(ccf.array.round(1).tolist(), [None] * 16 +  [-1.6, -1.7, -1.8, -1.9])
    ...
    def test_control_column_force_capt_real_data_1(self):
        ...
        fo = ControlColumnForce('Control Column Force (FO) Recorded',
                                ...
                                frequency=2)
        ccf.derive(capt, fo)
        self.assertEqual(np.ma.clump_unmasked(ccf.array), [slice(x, y) for x, y in [(582, 790), (800, 1032), (2596, 2739), (15166, 15326), (15546, 15592), (15616, 16224)]])
...
class TestControlColumnForceFO(unittest.TestCase):
    ...
    def test_control_column_force_fo(self):
        ...
        self.assertEqual(ccf.array.round(decimals=1).tolist(), [3] * 29 + [None] * 51)
    ...
    def test_control_column_force_fo_real_data_1(self):
        ...
        fo = ControlColumnForce('Control Column Force (FO) Recorded',
                                ...
                                frequency=2)
        ccf.derive(capt, fo)
        self.assertEqual(np.ma.clump_unmasked(ccf.array), [])
...
class TestLatitudeAndLongitudePrepared(unittest.TestCase):
    def test_can_operate(self):
        ...
        # only lat long
        self.assertTrue(('Latitude','Longitude',
    ...
    def test_latitude_smoothing_basic(self):
        ...
        smoother.get_derived([lat,lon]+[None]*10)
...
class TestMach(unittest.TestCase):
    def test_can_operate(self):
        self.assertEqual(Mach.get_operational_combinations(ac_type=helicopter), [])
...
class TestAirspeedMinusV2(unittest.TestCase, NodeTest):
    ...
    def setUp(self):
        ...
        self.operational_combinations = [
            ('Airspeed', 'V2 At Liftoff', 'Liftoff', 'Climb Start'),
            ('Airspeed', 'Airspeed Selected At Liftoff', 'Liftoff', 'Climb Start'),
            ('Airspeed', 'V2 At Liftoff', 'Airspeed Selected At Liftoff', 'Liftoff', 'Climb Start'),
            ('Airspeed', 'Airspeed Selected At Liftoff', 'V2 Lookup At Liftoff', 'Liftoff', 'Climb Start'),
            ('Airspeed', 'V2 At Liftoff', 'Airspeed Selected At Liftoff', 'V2 Lookup At Liftoff', 'Liftoff', 'Climb Start'),
        ]
    ...
    def test_derive__recorded_masked(self):
        node = self.node_class()
        node.derive(self.airspeed, KPV(items=[]), None, self.v2_lookup, self.liftoffs, self.climbs)
        expected = np.ma.repeat((0, 7, 0), (180, 820, 1000))
        expected[expected == 0] = np.ma.masked
        ma_test.assert_masked_array_equal(node.array, expected)
    ...
    def test_derive__recorded_and_lookup_masked(self):
        node = self.node_class()
        node.derive(self.airspeed, KPV(items=[]), None, KPV(items=[]), self.liftoffs, self.climbs)
