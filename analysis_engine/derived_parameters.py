...
##class MinSlatRetractionSpeed(DerivedParameterNode):
    ...
    ##def derive(self,
        ...
        ##valid = np.ma.logical_or(conf.array == '1', conf.array == '1*', conf.array == '1+F')
...
##class MinFlapRetractionSpeed(DerivedParameterNode):
    ...
    ##def derive(self,
        ...
        ### FIXME: Lookup a value for kac!
        ##valid = np.ma.logical_or(conf.array == '2', conf.array == '2*', conf.array == '3')
        ##self.array = kac * mp.ma.sqrt(gw.array / 1000)
        ##self.array[~valid] = np.ma.masked
...
class Airbrakes(DerivedParameterNode):
    units = ut.DEGREE
    
    def derive(self,
               sl=P('Spoiler (L) (5)'),  #sl=P('Spoiler (4)'),
               sr=P('Spoiler (R) (5)')): #sr=P('Spoiler (8)'),
        ...
        self.array = (src + slc + np.ma.abs(src - slc)) / 2
        self.array[self.array > 0] = 0.0  # should always be non-positive...        
...
class VLS(DerivedParameterNode):
    ...
    def can_operate(cls, available, manufacturer=A('Manufacturer'),
        ...
        spoilers = all_of((
            'Spoiler (L) (5)', #'Spoiler (4)',
            'Spoiler (R) (5)', #'Spoiler (8)',
        ), available)
        ...
        t0 = at.get_kaf_map(model.value, series.value, family.value)
        t1 = at.get_vls1g_map(model.value, series.value, family.value, engine_manufacturer.value)
    ...
    def derive(self,
        ...
        kaf_table = at.get_kaf_map(model.value, series.value, family.value)
        vls1g_table = at.get_vls1g_map(model.value, series.value, family.value,
                                       engine_manufacturer.value)
        ...
        descent = self.array.copy().astype(np.bool)
        descent.fill(False)
        ...
        vls02g = 187 + (gwt - 60) * 2
        vls02g[alt_std.array > 15000] += 1.1 * alt_std_minus_15k[alt_std.array > 15000] / 1000
        ...
        self.array[clean] = (np.ma.maximum(vls02g, vlslow) + dvls)[clean]
        ...
        self.array[~clean] = (vls1g * np.ma.sqrt(gwt + dgw))[~clean]
